...
# Custom JSON encoder para tratar inteiros do numpy
...
document.addEventListener('DOMContentLoaded', () => {{
    ...
    // Novos elementos do DOM para sugestões
    ...
    // --- LÓGICA DA NOVA TABELA DE SUGESTÕES ---
    function populateSuggestionFilters() {{
        // Popula filtro de Local
    ...
    function renderSuggestionsTable() {{
        ...
        suggestionsTableBody.innerHTML = ''; // Limpa a tabela
    ...
    function renderComparisonChart(questionId, locationId) {{
        ...
        if (localQuestionData && globalQuestionData) {{
            ...
            document.getElementById('local-chart-title').textContent = `Comparativo: ${{locationId}} (${{totalLocal.toLocaleString('pt-BR')}} resp.) vs. Rede (${{totalGlobal.toLocaleString('pt-BR')}} resp.)`;
    ...
    function populateComparisonSelects() {{
        ...
        const finalLocations = locations.includes(smeDefault) ? [smeDefault, ...locations.filter(loc => loc !== smeDefault).sort()] : locations.sort();
    ...
    function init() {{
        // Inicialização dos gráficos e tabelas principais
        ...
        // Inicialização da Tabela de Participação
        if (surveyData.participationData && surveyData.participationData.length > 0) {{
        ...
        // Inicialização da Nova Tabela de Sugestões
        if (surveyData.suggestionsData && surveyData.suggestionsData.length > 0) {{
            ...
            suggestionsSection.style.display = 'none'; // Esconde a seção se não houver dados
        }}
    }}
